...
- Conviviale mais utile
- Détaillée (n'hésite pas à développer en plusieurs phrases){loc_ctx}
...
IMPORTANT: Si des résultats de recherche te sont fournis, tu DOIS les utiliser pour répondre. Ne dis PAS que tu n'as pas accès aux données en temps réel si les résultats de recherche sont disponibles."""
    ...
    def search_pmu(self, race_info: str) -> str:
        """Recherche et formate les pronostics PMU (hors conversation)"""
        if self.api_type != "groq":
            return self.chat(race_info, voice=False)
        query = f"{race_info} turf pronostic geny.com zone-turf equidia.fr cheval-france"
        print(f"Chat: Recherche PMU ({query})...")
        results = self._search_web(query, max_results=5)
        import requests, time
        api_key = self.manager.app.config.get("groq_api_key", "")
        model = self.model or "llama-3.3-70b-versatile"
        pmu_prompt = (
            "Tu es un expert PMU. Tu DOIS répondre UNIQUEMENT avec des données réelles issues des résultats de recherche ci-dessous.\n"
            "Format de réponse OBLIGATOIRE:\n"
            "Pour chaque cheval: NUMÉRO - NOM (cote: X.X)\n"
            "Ne donne pas de conseils génériques. Ne dis PAS que tu n'as pas accès aux données en temps réel.\n"
            "Si les résultats de recherche ne contiennent pas les cotes, donne au moins les numéros et noms disponibles."
        )
        user_msg = f"Infos course: {race_info}\n\nRésultats recherche:\n{results}\n\nDonne le pronostic quinté avec numéros, noms et cotes."
        messages = [
            {"role": "system", "content": pmu_prompt},
        ...
        try:
            ...
            print(f"Groq PMU error ({r.status_code}): {r.text[:200]}")
        except Exception as e:
            print(f"Groq PMU error: {e}")
        return ""
